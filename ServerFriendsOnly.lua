...
local wrongTargetMessage = "This version of Server Friends Only was packaged for World of Warcraft " .. intendedWoWProjectName[intendedWoWProject] ..
...
local function FillFriendsTable()
	...
	if(db.showBNetFriends) then
		for i=1,bNetNumFriends do
			local hasFocus, characterName, client, realmName, realmID, faction, race, class, guild, zoneName, level, gameText, broadcastText, broadcastTime, canSoR, bnetIDGameAccount, presenceID, _, _, characterGUID = BNGetFriendGameAccountInfo(i, 1)
...
loadFrame:SetScript("OnEvent", function(self, event, isLogin, isReload)
	if isLogin or isReload then
		...
		for i=1,9 do
			CreateFrame("Button","ServerFriendsOnlyEntry"..i,ServerFriendsOnlyScrollBar,"ServerFriendsOnlyEntryTemplate","BackdropTemplate")
		end
		--Create header
		...
		hiddenOptionsButton = CreateFrame("Button", "ServerFriendsOnlyOptionsButton", FriendsTabHeader, "ServerFriendsOnlyEntryTemplate" and "BackdropTemplate")
